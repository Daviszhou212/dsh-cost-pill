	...
	factory: (require) => {
		...
		if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify(STYLE_ID) + ']') === null) {
			...
			style.textContent = [
				...
				// 合并态：钉在统计行最右端 —— order 压过官方节点的 DOM 顺序（React 重排也带不走），
				// margin-left:auto 在左对齐的行里把整组推到最右。
				'.dcp_root[data-merged=true]{width:auto;max-width:none;margin:0 0 0 auto;padding:0;flex:none;order:999}',
