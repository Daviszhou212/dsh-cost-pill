 ...
 *   2. 在 web server 上注册 **loopback-only** 路由：
 *      - `/api/cost-pill/balance`：提供商账户余额（Key 只在宿主进程内使用）；
 *      - `/api/cost-pill/tree`：**含整棵子代理树**的会话费用汇总 —— 子代理是独立会话，
 *        投影只覆盖本会话自己的日志，而子代理烧的是同一个余额，必须单独汇总。
 *
 * 为什么余额/树不也走投影：投影的契约是「对已提交事件做同步纯折叠」，不能做 I/O，
 * 也看不到其他会话的日志。账户余额是外部状态；子代理属于别的会话 —— 两者都走只读
 * 的精确路由；API Key 只在宿主进程内解析与使用，**绝不**回传浏览器。
 *
...
/** 含子代理树的费用汇总路由（精确路由，落在 /api 下）。 */
export const TREE_PATH = '/api/cost-pill/tree'
...
/**
 * 把任一 schema 实例对齐到注册表契约（`.parse(value)`）。
 *
 * 注册表只调用 `.parse`（zod 式），而 schemastery 的实例是**可调用函数、没有 .parse**
 * （校验靠 `schema(value)` 调用形式，非法输入抛错）——直接把实例交给注册表，会在
 * 检查点恢复与视图重算处抛 `TypeError`，炸掉整份会话投影快照。因此可调用的实例
 * 一律包一层 `.parse` 委托；既保留了真实校验，又满足注册表契约。
 */
function toContract(schema) {
	if (schema === null || schema === undefined) return passthroughSchema()
	if (typeof schema.parse === 'function') return schema
	if (typeof schema === 'function') {
		return {
			parse: (value) => schema(value),
			toString: () => 'schemastery(callable)'
		}
	}
...
function buildSchemas(Schema) {
	...
	return {
		stateSchema: toContract(stateSchema),
		viewSchema: toContract(viewSchema)
	}
}
...
/** 归一化插件行 config 里的 `balance` 段。timeoutMs 必须 > 0（0 会让 AbortSignal 立即中止，
 *  余额从此永久 unavailable 且没有任何告警——比缺省更糟）。 */
function resolveBalanceConfig(raw) {
	...
	return {
		...
		cacheMs: nonNegative(source.cacheMs, BALANCE_DEFAULTS.cacheMs),
		timeoutMs: positive(source.timeoutMs, BALANCE_DEFAULTS.timeoutMs),
		lowThreshold: nonNegative(source.lowThreshold, BALANCE_DEFAULTS.lowThreshold),
		resolveTimeoutMs: positive(source.resolveTimeoutMs, BALANCE_DEFAULTS.resolveTimeoutMs)
	}
}

/** 从 harness settings 的 `llm-deepseek` 命名空间 + credentials 缝解析连接事实。 */
async function deepseekFacts(ctx, resolveTimeoutMs) {
	...
	if (credentials !== undefined && credentials !== null && typeof credentials.resolve === 'function') {
		let timer = null
		try {
			// 凭据缝可能等待交互而永不 settle：不给 resolve 加超时，单飞闸里的 promise
			// 会永远挂着，余额路由到进程重启前都无响应（含 ?refresh=1）。
			const hit = await Promise.race([
				credentials.resolve(apiKeyEnv),
				new Promise((_resolve, reject) => {
					timer = setTimeout(() => reject(new Error('credentials resolve timeout')), resolveTimeoutMs)
				})
			])
...
function createBalanceSource(ctx, config) {
	...
	const load = async () => {
		...
		if (factsCache === null || Date.now() - factsCache.at > 300_000) {
			factsCache = { facts: await deepseekFacts(ctx, config.resolveTimeoutMs), at: Date.now() }
		}
...
export async function apply(ctx, config) {
	...
	// 余额路由与树路由是两个独立功能：关掉余额（balance.enabled=false）只应下线余额，
	// 不能顺带杀掉子代理树汇总 —— 那会让 pill 永远回退到「仅本会话」的费用。
	...
	// —— 含子代理树的费用汇总（loopback-only）——
	// 投影只覆盖本会话日志；子代理是独立会话、烧同一个余额。这条路由把「本会话 + 整棵
	// 子代理树」折出来交给浏览器。折叠按日志 mtime 增量缓存，轮询成本 = 20 次 stat。
	const sessionDirs = new Map() // 会话 id → 工作区目录
	const foldCache = new Map() // 日志路径 → { mtimeMs, state }
	const readTree = createTreeSource(sessionDirs, foldCache, config?.subagents, holder)
	ctx.effect(
		() =>
			ctx.webServer.register({
				kind: 'exact',
				...
				handler: async (req, res) => {
					...
					const url = new URL(req.url ?? TREE_PATH, 'http://localhost')
					const sessionId = url.searchParams.get('session') ?? ''
					if (sessionId === '') {
						writeJson(res, 400, { ok: false, error: 'missing session' })
						return
					}
					writeJson(res, 200, await readTree(normalizeSessionId(sessionId)))
				}
			}),
		'cost-pill: tree route'
	)
}

//#region 子代理树汇总

function createTreeSource(sessionDirs, foldCache, subagentConfig, holder) {
	const includeSubagents = subagentConfig === undefined || subagentConfig === null ? true : subagentConfig !== false
	return async function readTree(rootId) {
		if (!includeSubagents) return { ok: true, sessionId: rootId, includeSubagents: false }
		// 定位该会话的工作区目录（首次之后缓存）
		let workspace = sessionDirs.get(rootId)
		if (workspace === undefined) {
			workspace = findWorkspaceDir(sessionsRoot(), rootId)
			if (workspace !== undefined) sessionDirs.set(rootId, workspace)
		}
		if (workspace === undefined) {
			return { ok: false, error: 'workspace-not-found', sessionId: rootId }
		}

		// 工作区里所有会话的头部（构建父子树用），60 秒缓存
		const now = Date.now()
		let headers = headerCache.get(workspace)
		if (headers === undefined || now - headers.at > 60_000) {
			headers = { at: now, sessions: listWorkspaceSessions(workspace) }
			headerCache.set(workspace, headers)
		}

		const treeIds = collectTree(headers.sessions, rootId)
		let total = 0
		let subagentCost = 0
		let subagentCount = 0
		let own = null
		...
		for (const id of treeIds) {
			const logPath = sessionLogFile(workspace, id)
			let mtimeMs
			try {
			...
			const cachedFold = foldCache.get(logPath)
			let state
			if (cachedFold !== undefined && cachedFold.mtimeMs === mtimeMs) {
				state = cachedFold.state
			} else {
				try {
					state = foldFile(logPath, initialState())
				} catch {
					continue
				}
				foldCache.set(logPath, { mtimeMs, state })
			}
			const entryView = viewOf(state, holder.pricing, holder.meta)
			if (id === rootId) own = { key: id, ...entryView }
			else {
				subagentCost += entryView.cost
				subagentCount += 1
				members.push({ key: id, cost: entryView.cost, samples: entryView.samples })
			}
			total += entryView.cost
		}
		...
		return {
			...
			sessionId: rootId,
			includeSubagents,
			own: own === null ? null : { cost: own.cost, samples: own.samples },
			subagents: { count: subagentCount, cost: subagentCost, members: members.slice(0, 20) },
			total,
			// 兼容：客户端把 total 当「费用」显示，把 samples 当「有没有样本」判断
			samples: own === null ? 0 : own.samples,
			fetchedAt: now
		}
	}
}

const headerCache = new Map() // 工作区目录 → { at, sessions }
