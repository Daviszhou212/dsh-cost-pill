...
/** 会话日志文件名，按新旧探测：DSH 0.1.7-alpha.2 起写 v4（v3 是旧版遗留，只读不写）。 */
const LOG_FILENAMES = ['session.v4.jsonl.zstd', 'session.v3.jsonl.zstd']
 ...
 * 探测；日志文件名同样有两代（v4 现行、v3 遗留），v4 优先。都没命中（日志还没落盘
 * 的活会话）时返回裸 id 的规范 v4 路径，让调用方按「文件不存在」处理。
 */
export function sessionLogFile(workspaceDir, sessionId) {
	...
	for (const name of [bare, `session-${bare}`]) {
		for (const file of LOG_FILENAMES) {
			const logPath = join(workspaceDir, name, file)
			try {
	...
	return join(workspaceDir, bare, LOG_FILENAMES[0])
}
 ...
 * 日志文件名两代都认（v4 现行、v3 遗留），同目录两者并存时 v4 优先 —— v3 是升级前
 * 的旧内容，继续增长的是 v4。 */
export function listWorkspaceSessions(workspaceDir) {
	...
	for (const entry of readdirSync(workspaceDir, { withFileTypes: true })) {
		...
		let logPath
		for (const file of LOG_FILENAMES) {
			const candidate = join(workspaceDir, entry.name, file)
			try {
