...
/** 价目表的核对日期（官方定价页原文，见 DEFAULT_PRICING 注释）。 */
export const PRICING_CHECKED_AT = '2026-09-10'
...
export function resolvePricing(overrides, onlineModels) {
	...
	for (const [key, value] of Object.entries(seed)) {
		merged[key] = {
			offpeak: { ...(value.offpeak ?? value) },
			peak: { ...(value.peak ?? value.offpeak ?? value) }
		}
	}
	...
	for (const [key, entry] of Object.entries(overrides)) {
		...
		if (offpeakGiven || peakGiven) {
			// 分时段写法：**只给一个时段也成立**，另一个时段继承基座。
			// （早期版本要求两个都给，否则整条覆盖被当成「不分时段」静默失效。）
			merged[key] = {
				offpeak: sanitizeRates(offpeakGiven ? entry.offpeak : baseOffpeak, baseOffpeak),
				peak: sanitizeRates(peakGiven ? entry.peak : basePeak, basePeak)
			}
		} else {
